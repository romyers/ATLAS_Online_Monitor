...
namespace MuonReco {
  ...
  RTParam::RTParam(ConfigParser cp) : RTParam() {
    ...
    maxLin       = cp.items("AutoCalibration").getDouble("MaxLinear", -999, 0) * 2 - 1;
    // max linear is in percentage in config file and then we convert to chebyshev units
  }
  ...
  double RTParam::Eval(double time) {
    ...
    else if (time < maxLin)return (func->Eval(maxLin)-func->Eval(-1))*(time+1)/(maxLin+1);
