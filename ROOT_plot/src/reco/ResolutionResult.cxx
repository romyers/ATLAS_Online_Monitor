...
namespace MuonReco {
  ...
  ResolutionResult::ResolutionResult(ConfigParser cp, int _runN) {
    geo = new Geometry();

    runN     = cp.items("General").getInt("RunNumber", _runN, 0);
  ...
  void ResolutionResult::PopulateResVsRadius() {
	...
	Observable sigma(0,0);
	if (sigma_n.val < 30) {
	  sigma = sigma_w;
	}
	else if (sigma_w.val > 400) {
	  sigma = sigma_n;
	}
	else {
	  sigma = (sigma_n*amp_n + sigma_w*amp_w)/(amp_n + amp_w);
	}
  ...
  TH1* ResolutionResult::Deconvolute(ResolutionResult &other, Bool_t hitResiduals/*=kFALSE*/, Bool_t reconv/*=kFALSE*/) {
    ...
    for (double lower = 0; lower < maxRad; lower+=1) {
      ...
      if (py->GetEntries()) {
	Deconvolute(otherpy, py, TString::Format(name, (int)lower), reconv, kFALSE); // SAMESIGMA 
	// 
  ...
  void ResolutionResult::fitTH1DoubleGaussian(TH1* hist, Bool_t sameSigma) {
    ...
    singGaus->SetParameter(0, hist->GetStdDev());
    singGaus->SetParLimits(0, hist->GetStdDev()/2, hist->GetStdDev()*2);
    for (Int_t iBin=1; iBin<=hist->GetNbinsX(); ++iBin) {
      if (TMath::Abs(hist->GetBinContent(iBin))>1)
	hist->SetBinError(iBin, 3*TMath::Sqrt(TMath::Abs(hist->GetBinContent(iBin))));
      else
	hist->SetBinError(iBin, 1);
    }
    if (!sameSigma) hist->Fit(doubGaus, "B");
    //else hist->Fit("singGaus", "R");
  }
