...
namespace MuonReco {
  ...
  std::vector<TString> IOUtility::getDBFilePaths() {
    glob_t glob_result;
    memset(&glob_result, 0, sizeof(glob_result));

    int return_value = glob("output/run*/module*.root", GLOB_TILDE, NULL, &glob_result);
    
    std::vector<TString> paths;
    if (return_value == 0) {
      for (size_t i=0; i<glob_result.gl_pathc; ++i) {
	TString a = TString(glob_result.gl_pathv[i]);
	a.ReplaceAll(".root", "");
	TObjArray* Atoks = a.Tokenize("module");
	int aNum = ((TObjString*)(Atoks->Last()))->GetString().Atoi();
	delete Atoks;
	if (aNum)
    ...
    std::sort(paths.begin(), paths.end(), 
		...
		a.ReplaceAll(".root", "");
                b.ReplaceAll(".root", "");
		TObjArray* Atoks = a.Tokenize("module");
		TObjArray* Btoks = b.Tokenize("module");
		int aNum = ((TObjString*)(Atoks->Last()))->GetString().Atoi();
		int bNum = ((TObjString*)(Btoks->Last()))->GetString().Atoi();		
		delete Atoks;
		delete Btoks;
		return aNum < bNum;
	      });
