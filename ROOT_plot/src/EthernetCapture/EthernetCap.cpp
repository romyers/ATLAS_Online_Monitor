...
EthernetCap::EthernetCap(ostream &out) : outputStream(out) {
...
void EthernetCap::binary_write(const struct pcap_pkthdr* pkthdr, 
    ...
    for(int i=EthernetCap::DATA_START;i<EthernetCap::DATA_START+length;i+=5){
        if(*(packet_data+i)!=EthernetCap::IDLE_WORD){
            ...
            /*
            char word[5];
            memcpy(word, packet_data + i, 5);
            cout << static_cast<bitset<5>>(word);
            */
            // fwrite(packet_data+i,1,5,fp_binary_);
            // outputStream.write(packet_data + i, 5);

            // TODO: Graceful handling of case where outputStream is in a 
            //       failed state
